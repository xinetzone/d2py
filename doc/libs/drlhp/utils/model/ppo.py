...
from .general import export_plot, plot_correlation # get_logger, Progbar, 
# from .baseline_network import BaselineNetwork
from .network_utils import np2torch #build_mlp, device, 
...
class PPO(PolicyGradient):
    ...
    def update_policy(self, observations, actions, advantages, old_logprobs):
        ...
        log_probs = self.policy.action_distribution(observations=observations).log_prob(actions)
        r = torch.exp(log_probs - old_logprobs)
        r_clipped = torch.clip(r, 1 - self.eps_clip, 1 + self.eps_clip)
        loss = -torch.min(r * advantages, r_clipped * advantages).mean()

...
if __name__ == "__main__":
    ...
    all_results = {"100": [], "1000": [], "10000" : []}
    for seed in seeds:
        format_str = f"{gym_env_name}-0.05-{{}}-seed={seed}"
        all_results["1000"].append(
            np.load(directory / format_str.format("1000") / "scores.npy")
        )
        all_results["100"].append(
            np.load(directory / format_str.format("100") / "scores.npy")
        )
        all_results["10000"].append(
            np.load(directory / format_str.format("10000") / "scores.npy")
        )
    ...
    plt.savefig(directory / f"results-0.1-askfreq-{args.env_name}.png", bbox_inches="tight")
